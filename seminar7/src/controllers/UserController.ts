...
const createUser = async (req: Request, res: Response) => {
    ...
    try{//async 를 사용하려면 try catch 사용해야함
...
const signInUser = async(req: Request, res: Response) =>{
    ...
    try {
        ...
        else if (result === 401) return res.status(statusCode.UNAUTHORIZED).send(util.fail(statusCode.UNAUTHORIZED, message.INVALID_PASSOWRD));
...
const deleteUser = async (req: Request, res: Response) =>{
    ...
    try {
        ...
        console.log(error);
        //서버 내부에서 오류 발생
        res.status(statusCode.INTERNAL_SERVER_ERROR).send(util.fail(statusCode.INTERNAL_SERVER_ERROR, message.INTERNAL_SERVER_ERROR));//send 안에는 json 형식
    }
}
